...
$arguments = "-oldStyle", "-returntargetcode", "-register:user", "`"-filter:+[*]* -[Stashbox.Tests]* -[Stashbox]*.Utils*`"", "-target:dotnet.exe", "`"-targetargs:test $testPath --no-build`"", "-output:coverage.xml", "-skipautoprops", "-hideskipped:All"
